...
export function GameSidebar({
    ...
    return (
        <aside className={`${isNarrow ? 'w-full h-auto border-b flex-row items-start' : 'w-80 border-r flex-col h-full'} bg-slate-900/50 border-slate-800 p-2 flex gap-4 z-10 backdrop-blur-sm shrink-0 transition-all`}>
            ...
            <div className={`flex-1 flex flex-col min-h-0 bg-black/20 rounded-xl p-2 border border-slate-800/50 ${isNarrow ? 'h-44 overflow-hidden' : 'flex-1 overflow-hidden'}`}>
